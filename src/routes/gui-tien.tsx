...
function GuiTienPage() {
  ...
  const handleSpinnerDone = async () => {
    ...
    try {
      const updated = await deposit(selectedAccount.MaSTK, Number(depositAmount));
      if (updated) {
        toast.success(`Gửi tiền thành công! Số dư mới: ${formatCurrency(updated.SoDu)}`);
        setSelectedAccount(updated);
        setDepositAmount("");
      } else {
        toast.error("Không lấy được dữ liệu sổ sau khi gửi tiền.");
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : "Có lỗi xảy ra. Vui lòng thử lại.";
      toast.error(message);
